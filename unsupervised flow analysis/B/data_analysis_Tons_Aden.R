...
wsFile_Tonsil <- "~/Box/My Box Notes/4.Flowjo manual gating/B cell gating for Tonsil, Adenoid and PBMC/Tonsil_37c_B_cells.wsp"
...
sampleFCS_path_Tonsil <- sampleFCS_path_Tonsil %>%
  gsub(pattern = "/Volumes/Backup Plus/Backup Covid19 data/Data files for 37c reunmix/37c Tonsils reumix/", 
     replacement = "~/Box/37c Tonsils reumix/",
     x = .,
     ignore.case = FALSE,
     perl = FALSE,
     fixed = TRUE,
...
dir.create(path = "~/Desktop/gs_Tonsil")

flowWorkspace::save_gs(
  gs = gs_Tonsil,
  path = "~/Desktop/gs_Tonsil")

gs_Tonsil <- flowWorkspace::load_gs(
  path = "~/Desktop/gs_Tonsil")
...
# open FlowJo workspace
wsFile_Adenoid <- "~/Box/My Box Notes/4.Flowjo manual gating/B cell gating for Tonsil, Adenoid and PBMC/37c_Adenoids_B_Cells_after_shutdown_exclusion-80-78-35-39-52-rightlabel.wsp"
...
sampleFCS_path_Adenoid <- sampleFCS_path_Adenoid %>%
  grep(pattern = "/Volumes/Backup Plus/Backup Covid19 data/", 
       x = .,
       ignore.case = FALSE,
       perl = FALSE,
       value = TRUE,
...
metacells_Tonsil <- metacells_Tonsil[!names(metacells_Tonsil) %in% 
                                       outliers]

metacells_Adenoid <- metacells_Adenoid[!names(metacells_Adenoid) %in% 
                                         outliers]
...
# updated on 121220: save plots as pdf for editing in illustrator and change pop names
library(ggrepel)
#library(scattermore)
library(tidyverse)
...
plotModelRes(mod.list = mod_Adenoid,
  ...
  egg::set_panel_size(width = unit(4, "in"),
                      height = unit(4, "in")) %>%
  gridExtra::grid.arrange()  %>%
  ggsave(filename = "stats.Adenoid_B.pdf",
         width = 14,
         height = 6,
         useDingbats = FALSE)
...
plotModelRes(mod.list = mod_Tonsil,
  ...
  egg::set_panel_size(width = unit(4, "in"),
                      height = unit(4, "in")) %>%
  gridExtra::grid.arrange()  %>%
  ggsave(filename = "stats.Tonsil_B.pdf",
         width = 14,
         height = 6,
         useDingbats = FALSE)
...
top.prot.HM[[4]] %>%
  gridExtra::grid.arrange() %>%
  ggsave(filename = "211223_HM_markers_Tons_Aden_B.pdf",
         width = 5,
         height = 3,
         useDingbats = FALSE)
...
markers %>%
  ...
  do.call(gridExtra::grid.arrange,
          .) %>%
  ggsave(filename = "211226_UMAP_markers_Tons_Aden_B.pdf",
         width = 15,
         height = 15,
         useDingbats = FALSE)
