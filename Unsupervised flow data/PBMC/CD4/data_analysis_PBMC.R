...
wsFile <- "~/Box/My Box Notes/4.Flowjo manual gating/03-Mar-2021 37c PBMC after EXCLUTION.wsp"
...
plotModelRes(mod.list = mod,
  ...
  egg::set_panel_size(width = unit(4, "in"),
                      height = unit(4, "in")) %>%
  gridExtra::grid.arrange()  %>%
  ggsave(filename = "stats.PBMC_CD4.pdf",
         width = 14,
         height = 6,
         useDingbats = FALSE)
...
top.prot.HM[[4]] %>%
  gridExtra::grid.arrange() %>%
  ggsave(filename = "211223_HM_markers_PBMC_CD4.pdf",
         width = 5,
         height = 3,
         useDingbats = FALSE)
...
markers %>%
  ...
  do.call(gridExtra::grid.arrange,
          .) %>%
  ggsave(filename = "211226_UMAP_markers_CD4.pdf",
         width = 15,
         height = 15,
         useDingbats = FALSE)
